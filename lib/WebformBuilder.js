    ...
    value: function onDrop(element, target, source, sibling) {
      ...
      if (key && group) {
        ...
        if (index !== -1) {
          ...
          var formioContainer = _toConsumableArray(source.formioContainer);

          info = formioContainer.splice(_lodash["default"].findIndex(source.formioContainer, {
            key: element.formioComponent.component.key
      ...
      if (target.formioContainer) {
        if (sibling) {
          ...
          if (index !== -1) {
            var _formioContainer = _toConsumableArray(target.formioContainer);

            _formioContainer.splice(index, 0, info);

            target.formioContainer = _formioContainer;
          }
        } else {
          var _formioContainer2 = _toConsumableArray(target.formioContainer);

          _formioContainer2.push(info);

          target.formioContainer = _formioContainer2;
        }
        ...
        if (data) {
          ...
          if (parent) {
            ...
            if (parent.formioContainer && !isParentSaveChildMethod) {
              ...
              var formioContainer = _toConsumableArray(parent.formioContainer);
