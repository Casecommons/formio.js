...
var PasswordStrengthAddon = /*#__PURE__*/function (_FormioAddon) {
  ...
  var _proto = PasswordStrengthAddon.prototype;

  _proto.log2 = function log2(value) {
  ...
  _proto.calculatePasswordEntropy = function calculatePasswordEntropy(passwordLength, charactersPoolSize) {
    return !passwordLength || !charactersPoolSize ? 0 : this.log2(Math.pow(charactersPoolSize, passwordLength));
  };

  _proto.calculatePasswordEntropyWords = function calculatePasswordEntropyWords(wordsCount) {
  ...
  _proto.checkBlackList = function checkBlackList(value) {
  ...
  _proto.handleRuleCheckResult = function handleRuleCheckResult(valid, validation, message, errors) {
    if (valid !== true) {
      errors.push({
  ...
  _proto.performChecks = function performChecks(value) {
    var _this2 = this;

    var errors = [];
    var charactersPoolSize = 0;
    this.rulesSettings.forEach(function (settings) {
      if (_this2.rules[settings.name]) {
        var rule = _lodash["default"].merge({}, _this2.rules[settings.name], settings);

        var valid = rule.check(value, settings.options || {});
        var message = settings.message || valid;
        charactersPoolSize += _this2.handleRuleCheckResult(valid, rule, message, errors);
      }
    });
    this.customRules.forEach(function (rule) {
      if (rule.check && typeof rule.check === 'string') {
        var valid = _this2.evaluate(rule.check, _this2.component.evalContext({
          value: value
        }), 'valid');
  ...
  _proto.checkValidity = function checkValidity(value) {
  ...
  _proto.handleBlackListCheckResult = function handleBlackListCheckResult(result) {
  ...
  _proto.insertContainer = function insertContainer(element, container) {
      ...
      return {
        length: {
          check: function check(value, options) {
            var minLength = options.minLength || _this4.component.component.validate.minLength || 6;
